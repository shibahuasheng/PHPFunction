...
Class Image {
    //验证码
    const   VERIFY_LENGTH = "4";
    const   VERIFY_WIDTH = 150;
    const   VERIFY_HEIGHT = 60;
    const VERIFY_BGCOLOR = "#F3FBFE";
    const   VERIFY_SIZE = 30;
    const VERIFY_COLOR = '#444444';
    const   VERIFY_FUNC = strtolower;
    const   VERIFY_NAME = "verify";
    const   VERIFY_SEED = "3456789aAbBcCdDeEfFgGhHjJkKmMnNpPqQrRsStTuUvVwWxXyY";
    const VERIFY_FONTFILE =  './Data/font.ttf';

    //水印
    //水印图路径
    const WATER_IMAGE = './Data/logo.png';
    //水印位置
    const WATER_POS = 9;
    //水印透明度
    const WATER_ALPHA = 60;
    //JPEG图片压缩比
    const WATER_COMPRESSION = 80;
    //水印文字
    const WATER_TEXT = 'HouDunWang.com';
    //水印文字旋转角色
    const WATER_ANGLE = 0;
    //水印文字大小
    const WATER_FONTSIZE = 30;
    //水印文字颜色
    const WATER_FONTCOLOR = '#670768';
    //水印文字字体文件(写入中文字时需使用支持中文的字体文件)
    const WATER_FONTFILE = './Data/font.ttf';
    //水印文字字符编码
    const WATER_CHARSET = 'UTF-8';
    /**
	 * 生成验证码
	...
	Static Public function verify ($length = NULL, $width = NULL, $height = NULL) {
		...
		$length = empty($length) ? self::VERIFY_LENGTH : $length;
		$width = empty($width) ? self::VERIFY_WIDTH : $width;
		$height= empty($height) ? self::VERIFY_HEIGHT : $height;
		$bgColor = self::VERIFY_BGCOLOR;
		$seed = self::VERIFY_SEED;
		$fontFile = self::VERIFY_FONTFILE;
		$size = self::VERIFY_SIZE;
		$fontColor = self::VERIFY_SIZE;
	...
	Static Public function water ($img, $water = '', $save = NULL) {
		...
		//初始化参数(放进框架时把常量换成 读取配置文件项 )
		$water = empty($water) ? self::WATER_IMAGE : $water;
		$pos = self::WATER_POS;
		$alpha = self::WATER_ALPHA;
		$compression = self::WATER_COMPRESSION;
